...
const form = useForm({
  item_code: props.masterItem?.item_code || '',
  item_name: props.masterItem?.item_name || '',
  category_item_id: props.masterItem?.category_item_id || null,
  aol_id: props.masterItem?.aol_id || '',
  item_category: props.masterItem?.item_category || '',
  buyer: props.masterItem?.buyer || '',
  ppn: props.masterItem?.ppn || 0,
  pph: props.masterItem?.pph || 0,
});

function submit() {
  form.patch(route('master-items.update', props.masterItem.id), {
    preserveScroll: true,
    onSuccess: () => {
      emit('close');
    },
  ...
  <Dialog :open="show" @update:open="closeModal">
    <DialogContent class="sm:max-w-[700px]">
      <DialogHeader>
        ...
        <DialogDescription v-if="masterItem">
          Editing: {{ masterItem.item_code }} - {{ masterItem.item_name }}
        </DialogDescription>
      </DialogHeader>
